...
bool TerrainClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* heightMapFilename, char* colorTextureFilename, char* normalMapFilename)
{
	bool result;
	...
	// Load the textures.
	result = LoadTextures(device, deviceContext, colorTextureFilename, normalMapFilename);
	if(!result)
	{
		return false;
	}
...
bool TerrainClass::LoadTextures(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* colorTexturefilename, char* normalMapFilename)
{
	bool result;
	...
	result = m_NormalMapTexture->Initialize(device, deviceContext, normalMapFilename);
	if(!result)
	{
		return false;
	}
